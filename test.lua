            ...
            pcall(function()
                ...
                local function Hop()
                    for r = 1, math.huge do
                        ...
                        game:GetService("Players").LocalPlayer.PlayerGui.ServerBrowser.Frame.Filters.SearchRegion.TextBox.Text = region
                        
                        local bP = game:GetService("ReplicatedStorage").__ServerBrowser:InvokeServer(r)
                        for k, v in pairs(bP) do
                            if k ~= game.JobId and v["Count"] < bO then
                                game:GetService("ReplicatedStorage").__ServerBrowser:InvokeServer("teleport", k)
                                return -- Đã teleport, thoát khỏi vòng lặp
                            end
                        end
                    end
                end
                ...
                if not getgenv().Loaded then
                    local function handleError(v)
                        if v.Name == "ErrorPrompt" and v.Visible and v.TitleFrame.ErrorTitle.Text == "Teleport Failed" then
                            HopLowServer() -- Gọi lại hàm nếu teleport thất bại
                            v.Visible = false
                        end
                    end
        
                    -- Kiểm tra các prompt hiện tại
                    for _, v in pairs(game.CoreGui.RobloxPromptGui.promptOverlay:GetChildren()) do
                        handleError(v)
                    end
                    
                    game.CoreGui.RobloxPromptGui.promptOverlay.ChildAdded:Connect(handleError)
                    getgenv().Loaded = true
                end
